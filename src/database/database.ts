...
import { ConfigParam, Param, logger } from 'commonBack';
import { DbManager } from 'class'
import { RequestGetDbitem } from 'api';

export const dbpath = './sample.db'
...
export let dbmgr: DbManager = null;
...
export function createTable(TableschemaQuery: string[]) {
  TableschemaQuery.forEach((query) => {
    database.run(query, (err: Error) => {
      if (err) {
        logger.info(`executing query: ${err.message}`);
      }
    });
  });
}
